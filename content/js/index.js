...
const prodPreviewDescriptionsENG = [
    window.jsTranslations.sweater_weather || `See if it's <a href="https://open.spotify.com/track/2QjOHCTQ1Jl3zawyYOpxh6?si=62dad11bc7384e79" target="_blank" class="accent-text"> sweater weather </a> outside`,
    window.jsTranslations.lessons_afternoon || `Check which <span class="accent-text">lessons</span> you have in the afternoon`,
    window.jsTranslations.your_bus_arrive || `Find out when <span class="accent-text">your bus</span> will arrive`,
    window.jsTranslations.homework_check || `Quickly check if you made your <span class="accent-text">homework</span>`
]
...
function updateFunPreview(shouldAnimate, extraFun) {
    ...
    if (extraFun) {
        ...
        document.getElementById("volume-text").innerHTML = window.jsTranslations.volume_turn_on || `Please turn on volume for the full experience`
        ...
        document.getElementById("volume-icon").addEventListener("click", function () {
            ...
            document.getElementById("volume-text").innerHTML = window.jsTranslations.volume_continue || `Press <span id="fun-continue-button" class="accent-text">here</span> to continue`
